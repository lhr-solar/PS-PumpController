...
StaticTask_t xBlinkyTaskBuffer_PDE;
StackType_t xBlinkyStack_PDE[configMINIMAL_STACK_SIZE];

StaticTask_t xMVPTestTaskBuffer;
StackType_t xMVPTestStack[configMINIMAL_STACK_SIZE];

EMC2305_HandleTypeDef chip1;
    ...
    while (1) {
        ...
        if (Temp_GetReading(&message, portMAX_DELAY) == TEMP_OK) {
            ...
            if (GetTempInt(message.temp_data) >= TEMP_THRESHOLD_MC) {
                EMC2305_SetFanPWM(&chip1, EMC2305_FAN1, PDE_FAN_PWM_TARGET); // setting both fans bc I don't remember which one's plugged in
                EMC2305_SetFanPWM(&chip1, EMC2305_FAN2, PDE_FAN_PWM_TARGET);
